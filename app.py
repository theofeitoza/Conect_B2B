...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'company_id' not in session: flash('Você precisa estar logado para acessar esta página.', 'error'); return redirect(url_for('login'))
        return f(*args, **kwargs)
    return decorated_function
def supplier_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if session.get('user_type') != 'supplier': flash('Acesso negado. Esta área é restrita para fornecedores.', 'error'); return redirect(url_for('dashboard'))
        return f(*args, **kwargs)
    return decorated_function
...
class Product(db.Model):
    # (sem alterações)
    id = db.Column(db.Integer, primary_key=True); name = db.Column(db.String(100), nullable=False); description = db.Column(db.Text, nullable=False); category = db.Column(db.String(80), nullable=False); base_price = db.Column(db.Float, nullable=True); supplier_id = db.Column(db.Integer, db.ForeignKey('company.id'), nullable=False); image_filename = db.Column(db.String(255), nullable=True)
    quote_requests = db.relationship('QuoteRequest', backref='product', lazy=True)

class QuoteRequest(db.Model):
    # ATUALIZAÇÃO NO STATUS: Adicionado "Aceito", "Recusado"
    id = db.Column(db.Integer, primary_key=True); quantity = db.Column(db.Integer, nullable=False); message = db.Column(db.Text, nullable=True); status = db.Column(db.String(50), nullable=False, default='Pendente'); timestamp = db.Column(db.DateTime, nullable=False, default=datetime.utcnow) # Status: Pendente, Respondido, Aceito, Recusado
...
# (Rotas de home, register, login, logout, products, add_product, product_detail permanecem inalteradas)
@app.route('/')
...
def register():
    if request.method == 'POST':
        ...
        db.session.add(new_company); db.session.commit(); flash('Empresa cadastrada com sucesso! Por favor, faça o login.', 'success'); return redirect(url_for('login'))
...
def login():
    if request.method == 'POST':
        ...
        if company and company.check_password(password):
            session['company_id']=company.id; session['company_name']=company.company_name; session['user_type']=company.user_type; flash(f'Login realizado com sucesso! Bem-vindo, {company.company_name}.', 'success'); return redirect(url_for('dashboard'))
        else: flash('E-mail ou senha inválidos.', 'error'); return redirect(url_for('login'))
    return render_template('login.html')
@app.route('/logout')
@login_required
def logout(): session.clear(); flash('Você saiu da sua conta com sucesso.', 'success'); return redirect(url_for('home'))
@app.route('/dashboard')
@login_required
def dashboard():
    company=Company.query.get(session['company_id'])
    if company.user_type == 'supplier':
        my_products=company.products; quotes=company.received_quotes.order_by(QuoteRequest.timestamp.desc()).all()
        return render_template('dashboard.html', products=my_products, quotes=quotes)
    else: quotes=company.sent_quotes.order_by(QuoteRequest.timestamp.desc()).all(); return render_template('dashboard.html', quotes=quotes)
@app.route('/products')
@login_required
def products(): all_products=Product.query.order_by(Product.id.desc()).all(); return render_template('products.html', products=all_products)
...
def product_detail(product_id):
    ...
    if request.method == 'POST':
        ...
        db.session.add(new_quote); db.session.commit(); flash('Solicitação de cotação enviada com sucesso!', 'success'); return redirect(url_for('dashboard'))
...
def add_product():
    if request.method == 'POST':
        ...
        filename=None
        if image_file and allowed_file(image_file.filename): filename=secure_filename(image_file.filename); image_file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        ...
        db.session.add(new_product); db.session.commit(); flash('Produto adicionado com sucesso!', 'success'); return redirect(url_for('dashboard'))
    return render_template('add_product.html')
@app.route('/quote/<int:quote_id>', methods=['GET', 'POST'])
@login_required
def quote_detail(quote_id):
    quote = QuoteRequest.query.get_or_404(quote_id)
    if session['company_id'] not in [quote.buyer_id, quote.supplier_id]: flash('Você não tem permissão para visualizar esta cotação.', 'error'); return redirect(url_for('dashboard'))
    if request.method == 'POST':
        ...
        offered_price_str = request.form.get('offered_price'); supplier_message = request.form.get('supplier_message')
        ...
        quote.offered_price=float(offered_price_str); quote.supplier_message=supplier_message; quote.status='Respondido'; quote.response_timestamp=datetime.utcnow()
        db.session.commit(); flash('Proposta enviada com sucesso!', 'success'); return redirect(url_for('dashboard'))
    return render_template('quote_detail.html', quote=quote)

# --- NOVAS ROTAS PARA ACEITAR/RECUSAR ---

@app.route('/quote/<int:quote_id>/accept', methods=['POST'])
@login_required
def accept_quote(quote_id):
    quote = QuoteRequest.query.get_or_404(quote_id)
    # Segurança: Apenas o comprador original pode aceitar
    if session.get('user_type') != 'buyer' or session.get('company_id') != quote.buyer_id:
        flash('Ação não permitida.', 'error')
        return redirect(url_for('dashboard'))
    
    quote.status = 'Aceito'
    db.session.commit()
    flash('Proposta aceita com sucesso! O fornecedor será notificado.', 'success')
    return redirect(url_for('dashboard'))
...
def decline_quote(quote_id):
    quote = QuoteRequest.query.get_or_404(quote_id)
    # Segurança: Apenas o comprador original pode recusar
    if session.get('user_type') != 'buyer' or session.get('company_id') != quote.buyer_id:
        flash('Ação не permitida.', 'error')
        return redirect(url_for('dashboard'))
